...
export default function Component() {
  ...
  const [selectedCampaign, setSelectedCampaign] = useState<CampaignRun | null>(
    null
  );
  ...
  const [existingCampaigns, setExistingCampaigns] = useState<CampaignRun[] | null>(
    null
  );
  ...
  const createCampaign = async (method: string) => {
    ...
    try {
      ...
      // Prepare common run parameters
      const runParams = {
        run_name: selectedCampaign 
          ? `${selectedCampaign.campaign_name} ${method}`
          : `${campaignName} ${method}`,
        platform: selectedPlatform,
        discovery_method: method,
        input_list: accounts.split("\n").map((acc) => (acc ?? "").trim()).filter(Boolean),
        prompt_id: selectedEvaluation,
        minimum_followers: filterFollowerEnabled && followerMin ? Number(followerMin) : undefined,
        maximum_followers: filterFollowerEnabled && followerMax ? Number(followerMax) : undefined,
        country_list: filterCountriesEnabled && countries.length > 0 ? countries : undefined
      };
      ...
      if(selectedCampaign){
        ...
        router.push(`/campaigns/${selectedCampaign.campaign_id}`);
      }else{
        ...
        const newCampaign: CampaignRun = response.data;
        ...
        if (newCampaign?.campaign_id) {
          ...
          router.push(`/campaigns/${newCampaign.campaign_id}`);
        }
      }
