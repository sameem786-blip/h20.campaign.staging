...
export function CreatorCard({ creator }: CreatorCardProps) {
  ...
  const getVideoPerformanceText = (): string => {
    try {
      if (!creator.platform_info?.video_analysis) return 'No video performance data available';
      
      const analysis = creator.platform_info.video_analysis as Record<string, unknown>;
      ...
      const distribution = analysis.last_15_videos_distribution_relative_to_median as VideoDistributionItem[] || [];
      const medianRange = distribution.find(d => d.range === '1–2×' || d.range === '1-2×');
  ...
  const getBrandFitText = (): { text: string; colorClass: string } => {
    if (creator.evaluation_score === 3) {
      return { text: "Excellent", colorClass: "text-green-600" };
    } else if (creator.evaluation_score === 2) {
      return { text: "Good", colorClass: "text-blue-600" };
    } else {
  ...
  const getOutlierRate = (): string => {
    try {
      ...
      const outliers = distribution.filter(d => 
        !['0.5–1×', '1–2×', '0.5-1×', '1-2×'].includes(d.range)
      );
      
      if (outliers.length > 0) {
        const totalPercentage = outliers.reduce((sum, item) => sum + item.percentage, 0);
  ...
  return (
    <>
          ...
          <div className="border rounded-lg shadow-sm bg-white w-full h-full flex flex-col">
            ...
            <div className="p-4 border-b">
              <div className="flex justify-between items-center">
                <div>
                  <div className="font-medium text-lg">{creator.username}</div>
                  <div className="text-sm text-gray-500">{creator.core_platform || 'Unknown Platform'}</div>
                </div>
                <button
                  className="py-2 px-3 text-center text-sm border border-gray-300 rounded-md hover:bg-gray-50"
            ...
            <div className="flex-1 grid grid-cols-2 gap-0">
              ...
              <div className="p-3 border-t border-r">
                <div className="flex justify-between items-center mb-2">
                  <h3 className="text-sm font-medium text-gray-700">Creator Metrics</h3>
                  <button>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <div className="font-medium text-sm">
                      <span className={`text-xs ${getBrandFitText().colorClass}`}>
                  ...
                  <div className="flex justify-between">
                    ...
                    <div className="font-medium text-sm">{formatNumber(creator.platform_info?.followers)}</div>
                  </div>
                  <div className="flex justify-between">
                    <div className="text-xs text-gray-500">Outlier Rate</div>
                    <div className="font-medium text-sm text-red-500">{getOutlierRate()}</div>
                  </div>
                  <div>
                    <div className="text-xs text-gray-500">Negotiation Summary</div>
                    <div className="mt-3 max-h-32 overflow-y-auto">
                      {negotiationsLoading ? (
                        ...
                        <div className="text-xs text-gray-500 py-2">No negotiation history available</div>
                      ) : (
                        <div className="space-y-2">
                          {negotiations.map((item) => (
                            <div
                              ...
                              className={`p-2 rounded text-xs ${
                                item.direction === 'inbound' ? 'bg-green-50 border-l-2 border-green-300' : 'bg-blue-50 border-l-2 border-blue-300'
                              }`}
                            >
                              <div className="flex justify-between mb-1">
                                <span className="font-medium text-xs">
                                  {item.direction === 'inbound' ? 'Creator' : 'Our Team'}
                                </span>
                                <span className="text-xs text-gray-500">{formatDate(item.created_at)}</span>
                              </div>
                              <p className="text-xs text-gray-700">{item.negotiation_summary}</p>
                            </div>
                          ))}
                        </div>
                      )}
              ...
              <div className="p-3 border-t">
                <div className="flex justify-between items-center mb-2">
                  <h3 className="text-sm font-medium text-gray-700">View Statistics</h3>
                  <button>
                    <svg className="h-4 w-4 text-gray-400" viewBox="0 0 20 20" fill="currentColor">
                      <path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" />
                    </svg>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <div className="font-medium text-sm">{formatNumber(videoSummary.total_views)}</div>
                  </div>
                  <div className="flex justify-between">
                    <div className="text-xs text-gray-500">Most Viewed</div>
                    <div className="font-medium text-sm">{formatNumber(videoSummary.most_viewed)}</div>
                  </div>
                  <div className="flex justify-between">
                    <div className="text-xs text-gray-500">Mean Views</div>
                    <div className="font-medium text-sm">{formatNumber(videoSummary.mean_views)}</div>
                  </div>
                  <div className="flex justify-between">
                    <div className="text-xs text-gray-500">Median Views</div>
                    <div className="font-medium text-sm">{formatNumber(videoSummary.median_views)}</div>
                  </div>
                  <div className="flex flex-col space-y-2">
                    ...
                    <div className="mt-3 max-h-32 overflow-y-auto space-y-2">
                      {ratesLoading ? (
                        ...
                        <div className="text-xs text-gray-500 py-2">No rates available</div>
                      ) : (
                        rates.map((rate) => (
                          <div key={rate.id} className="flex justify-between bg-green-50 p-2 rounded-md">
                            <div className="flex flex-col gap-2">
                              <div className="font-medium text-xs">{rate.name}</div>
                              <div className="text-xs text-gray-500">{rate.notes}</div>
                            </div>
                            <div className="flex items-center gap-2">
                              <div className="font-medium text-sm">{rate.price} {rate.currency} / {rate.unit}</div>
                            </div>
                          </div>
                        ))
                      )}
            ...
            <div className="p-3 border-t">
              <h3 className="text-sm font-medium text-gray-700 mb-1">Video Performance</h3>
              <div className="text-xs text-gray-600 mb-2">
                {getVideoPerformanceText()}
              </div>
              {/* Add Show Data Button */}
              {creator.platform_info?.video_analysis && (
                <button
                  className="w-full py-1.5 px-3 text-center text-xs border border-blue-300 rounded-md bg-blue-50 text-blue-600 hover:bg-blue-100"
      ...
      <VideoPerformanceModal
        videoAnalysis={creator.platform_info?.video_analysis as Record<string, unknown> || null}
