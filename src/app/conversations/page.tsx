...
export default function ConversationsPage() {
      ...
      try {
        ...
        if (data && data.length > 0) {
          ...
          data.forEach((conversation) => {
            if (
              conversation.last_message_tags &&
              Array.isArray(conversation.last_message_tags)
            ) {
              conversation.last_message_tags.forEach((tag: string) =>
                tags.add(tag)
              );
            }
          });
  ...
  const updateFilter = (
    filterName: keyof ConversationFilters,
    value: unknown
  ) => {
      ...
      <Card className="shadow-md border border-gray-200">
        <CardContent className="space-y-6">
                ...
                {availableTags.map((tag) => {
                  ...
                  return (
                    <Badge2
                      key={tag}
                      variant={
                        (isSelected ? color.base : color.subtle) as BadgeVariant
                      }
              ...
              <div className="flex flex-row space-y-1 gap-4">
                <div className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      updateFilter("followUpNeeded", checked)
                    }
                  />
                  <label
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      updateFilter("notApproved", checked)
                    }
                  />
                  <label
                    htmlFor="notApproved"
          ...
          <div className="flex justify-end">
            <Button
              className="bg-[#FF9A63] text-black cursor-pointer hover:bg-[#E3532C]"
                ...
                >
                  ...
                  <TableCell>
                    <Badge
                      className={cn(
                        "font-medium",
                        getStageColor(conversation.last_message_stage)
                      )}
                    >
